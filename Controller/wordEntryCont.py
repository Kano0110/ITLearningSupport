...
class wordEntryCont:
    ...
    def get_id_pass():
        ...
        Name = wordEntryView.entry_Name.get("1.0", tkinter.END)
        Kai = wordEntryView.entry_Kai.get("1.0", tkinter.END)

        print('単語名：',Name)
        print('解説：',Kai)
        print('カテゴリ：',wordEntryView.cb_Category)
        print('分野：',wordEntryView.cb_Bunya)
    ...
    def create_reset_window():
        ...
        wordEntryView.label_sub_kakunin = ttk.Label(sub_win,text="本当にリセットしますか？")

        # ボタン
        wordEntryView.button_erase = ttk.Button(sub_win,text="はい",command=lambda:run_reset())
        wordEntryView.button_return = ttk.Button(sub_win,text="いいえ",command=lambda:close_subWindow())
